		...
		for (int y = 0; y < 3; ++y)
		{
			for (int x = 0; x < 9; ++x)
			{
				this.addSlotToContainer(
						new Slot(player.inventory, x + y * 9 + 9, 8 + playerInvOffX + x * 18, 84 + playerInvOffY + y * 18));
			}
		}

		for (int x = 0; x < 9; ++x)
		{
			this.addSlotToContainer(new Slot(player.inventory, x, 8+ playerInvOffX + x * 18, 142+ playerInvOffY));
		}
	}
